...
- The site is hosted on **Cloudflare Pages** (confirmed by the repo owner), building from `main` via Cloudflare's own git integration — not GitHub Actions. That's why there's no deploy workflow here: pushing/merging to `main` is enough to trigger a production deploy on Cloudflare's side, outside this repo's visibility. Treat a merge to `main` as a real production deploy trigger, not an inert git operation — ask the user before merging or pushing to `main` unless they've already asked for that.
