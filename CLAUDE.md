...
There are tests — run with `npm run test`. Run a single test file with `npm run test src/animations.test.ts`. Type-checking is done via `astro check` (runs as part of `build`). Lefthook runs `biome check --write` on staged files automatically on pre-commit (fixed files are re-staged). Run `npx lefthook install` after cloning to activate hooks. Run Biome checks manually with `npx biome check src/`.
