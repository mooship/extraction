...
function renderSankeyDesktopChart(): void {
	...
	const leftLabels = leftFlows
		.map((flow) => {
			...
			return `<text x="85" y="${startY + 4}" fill="var(--fg)" font-size="8" paint-order="stroke" stroke="var(--bg)" stroke-width="3" stroke-linejoin="round">${flow.label}: ${flow.displayValue}</text>`;
		})
		.join("");
	...
	const rightLabels = rightFlows
		.map((flow) => {
			...
			return `<text x="75" y="${y + 4}" fill="var(--semantic-green)" font-size="8" paint-order="stroke" stroke="var(--bg)" stroke-width="3" stroke-linejoin="round">${flow.label}: ${flow.displayValue}</text>`;
		})
		.join("");
